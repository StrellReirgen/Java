...
<body class='bg-dark'>
	<div class='container text-center'>
		...
		<h3 class='text-success'>Bienvenido ${user.firstName} este es tu organizador de Eventos </h3>
		<form action="/logout" method="post">
			<input class="text-center col form-control btn btn-danger" type="submit" value="Cerrar Sesion"/>
		</form>
		</div>
		<c:if test="${events.size() > 0 || locales.size() > 0}">
			<c:if test="${locales.size() > 0}">
				<h6 class="text-light d-flex justify-content-start">Aqui estan los eventos en tu región</h6>
				<table class="table table-striped table-hover table-bordered">
					...
					<tbody>
						<c:forEach items="${locales}" var="local">
							<tr>
								<td><a class="btn btn-outline-primary border-0" href="/event/${local.id}">${local.name}</a></td>
								<td class="text-light"><fmt:formatDate pattern ="dd-MMMM-yyyy" value ="${local.date}"/></td>
								...
								<td class="d-flex justify-content-center"><c:choose>
     									<c:when test="${local.host.id == user.id}">
											...
											<form action="/event/${local.id}/delete" method="post">
												<input class="ml-2 btn btn-danger" type="submit" value="Borrar"/>
											</form>
									    </c:when>
									    <c:otherwise>
										    	...
										    	<c:when test="${invitado}">
										    		<form action="/event/${local.id}/cancel" method="post">
										    			<div class="d-flex justify-content-center">
															...
															<input class="ml-2 btn btn-danger" type="submit" value="Cancelar"/>
														</div>
													</form>
												</c:when>
												<c:otherwise>
													<form action="/event/${local.id}/join" method="post">
														<input class="btn btn-primary" type="submit" value="Unirse"/>
													</form>
												</c:otherwise>
											</c:choose>
									    </c:otherwise>    
			...
			<c:if test="${events.size() > 0}">
				<h6 class="text-light d-flex justify-content-start">Aqui hay eventos en otras regiones</h6>
				<table class="table table-striped table-hover table-bordered">
					<thead>
						<tr>
							<th class='text-primary'>Nombre</th>
					...
					<tbody>
						<c:forEach items="${events}" var="evento">
							<tr>
								<td><a class="btn btn-outline-primary border-0" href="/event/${evento.id}">${evento.name}</a></td>
								<td class="text-light"><fmt:formatDate pattern ="dd-MMMM-yyyy" value ="${evento.date}"/></td>
			...
			<div class="mt-4 col-4">
				...
				<form action="/event/new" method="post">
					...
					<div class="form-group d-flex justify-content-center">
						<input class="mt-1 text-center col-12 form-control" placeholder="Nombre" required="required" type="text" name="name"/>
					</div>	
					<div class="form-group d-flex justify-content-center">
						<input class="mt-1 text-center col-12 form-control" required="required" type="date" name="date"/>	
					</div>
					<div class="form-group d-flex justify-content-center">
						<input class="mt-1 text-center col-12 form-control" placeholder="Ciudad" required="required" type="text" name="city"/>
					</div>	
					<div class="form-group d-flex justify-content-center">
